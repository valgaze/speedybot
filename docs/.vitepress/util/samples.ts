...
const card = Bot.card()
    ...
    .addPickerDropdown(["Select Department"].concat(departments))
    .addText(
...
const card = Bot.card()
                ...
                .addText("**TIP:** Images can be links too")
...
const card = Bot.card()
                .addTitle("Chip Card")
                .addSubtitle("SpeedyCards have a special feature-- 'chips'")
                .addText('Chips are pretty simple but very useful')
                .addText('If you add a chip, when the user clicks it your bot will react as if the user uttered that phrase')
                .addChips(['ping', 'pong', { title: "ping 🏓", value: "ping" }, { title: "🏓 pong", value: "pong" }])
`,
  },
  {
    name: "format",
